...
const AdminLayout = ({ children, title, subtitle }) => {
  ...
  const menuItems = [
    { path: '/admin/dashboard', label: 'Dashboard', icon: <LayoutDashboard size={22} /> },
    // --- MENU BARU: KELOLA RAWAT INAP ---
    { path: '/admin/rawat-inap', label: 'Kelola Rawat Inap', icon: <Bed size={22} /> }, 
    // -------------------------------------
    { path: '/admin/medicines', label: 'Manajemen Obat', icon: <Pill size={22} /> },
    { path: '/admin/reports', label: 'Laporan & Analitik', icon: <BarChart3 size={22} /> },
    // { path: '/admin/users', label: 'Kelola User', icon: <Users size={22} /> }, // Opsional
    // { path: '/admin/settings', label: 'Pengaturan', icon: <Settings size={22} /> }, // Opsional
  ];
      ...
      >
        ...
        <div className={`h-16 flex items-center mb-4 ${isExpanded ? 'justify-between' : 'justify-center'}`}>
          ...
          {isExpanded && (
            <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
                <div className="w-8 h-8 bg-slate-800 rounded-lg flex items-center justify-center text-white shrink-0 shadow-sm">
                    <ShieldCheck size={20} />
                </div>
                <span className="text-xl font-bold tracking-tight text-slate-800">SIMRS <span className="text-slate-500 text-sm font-normal ml-1">Admin</span></span>
            </div>
          )}
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                {/* Icon */}
                <span className={`shrink-0 ${isActive ? 'text-slate-900' : 'text-slate-500'}`}>
                  {item.icon}
                </span>
        ...
        <div className={`mt-auto pt-4 border-t border-slate-200 ${isExpanded ? '' : 'flex flex-col items-center'}`}>
            <div className={`flex items-center gap-3 mb-2 ${isExpanded ? 'px-2' : 'justify-center'}`}>
                <div className="w-9 h-9 rounded-full bg-slate-800 text-white flex items-center justify-center font-bold text-sm shadow shrink-0">
                    A
                </div>
                {isExpanded && (
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-slate-700 truncate">Administrator</p>
                        <p className="text-xs text-slate-400 truncate">Super User</p>
                    </div>
                )}
            </div>
